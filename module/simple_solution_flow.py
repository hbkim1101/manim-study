...
def _highlight_units(report, source_units, target_units):
    ...
    for row in report:
        ...
        path = target.path
        source_is_subscript = any(
            "subscript" in source_units[index].path for index in row_source_ids
        )
        if (reason == "arithmetic-sequence-expansion" and source_is_subscript
                and "base" in path):
            continue
